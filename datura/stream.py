...
async def collect_final_synapses(
    ...
    bt.logging.debug(f"@@1 Starting collect_final_synapses with max_execution_time: {max_execution_time}")
    final_synapses = [None] * len(async_responses)

    if max_execution_time <= 60:
        bt.logging.debug(f"@@1 Max execution time is less than or equal to 60, processing in groups.")
        ...
        bt.logging.debug(f"@@1 Split async_responses into {len(async_responses_groups)} groups.")
        
        group_indices = list(range(len(async_responses_groups)))
        random.shuffle(group_indices)
        bt.logging.debug(f"@@1 Shuffled group indices: {group_indices}")
        ...
        for group_index in group_indices:
            ...
            bt.logging.debug(f"@@1 Processing group {group_index} with UIDs: {group_uids}")
